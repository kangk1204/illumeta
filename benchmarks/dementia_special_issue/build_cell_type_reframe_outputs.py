...
def guarded_markdown(summary: pd.DataFrame, generated_at: str, gse306227: pd.DataFrame | None = None) -> str:
    ...
    `GSE306226/Neurons_vs_Microglia_results` provides the local positive-control/reference axis for cell-type-aware framing [24,25]. It compares 20 neurons with 20 microglia, passed sample QC, and produced {row_g306.strict_consensus_dmps} strict plus {row_g306.native_consensus_dmps} native Minfi/SeSAMe consensus DMPs. Branch concordance was {row_g306.branch_concordance}. Because {row_g306.lambda_guard}, it should be used as a reference axis demonstrating that IlluMeta can recover a large neural cell-type contrast, not as an unqualified discovery claim.
    {gse306227_block}
...
def revise_markdown(original: str, summary: pd.DataFrame, generated_at: str, gse306227: pd.DataFrame | None = None) -> str:
    ...
    new_abstract = (
        ...
        f"As a neural cell-type reference axis, GSE306226 Neurons_vs_Microglia [24,25] produced {row_g306.strict_consensus_dmps} strict and {row_g306.native_consensus_dmps} native Minfi/SeSAMe consensus DMPs across 20 neurons and 20 microglia, but its lambda guard triggered. "
